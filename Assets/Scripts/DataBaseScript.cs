        ...
        foreach (CardInformation card in CardData) {
            if (card.aspects.Length > 0) {
                switch (card.aspects[0]) {
                    case 'W':
                        newCard = Instantiate(genericCard, abas.FindChild("Aba1").GetChild(0).GetChild(0));
                        newCard.GetComponent<AddCardInformation>().card = card;
                        break;

                    case 'E':
                        newCard = Instantiate(genericCard, abas.FindChild("Aba2").GetChild(0).GetChild(0));
                        newCard.GetComponent<AddCardInformation>().card = card;
                        break;

                    case 'F':
                        newCard = Instantiate(genericCard, abas.FindChild("Aba3").GetChild(0).GetChild(0));
                        newCard.GetComponent<AddCardInformation>().card = card;
                        break;

                    case 'A':
                        newCard = Instantiate(genericCard, abas.FindChild("Aba4").GetChild(0).GetChild(0));
                        newCard.GetComponent<AddCardInformation>().card = card;
                        break;
                }
