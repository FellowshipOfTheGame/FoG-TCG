	...
	public void Attack() {
        ...
        if (Board.currPlayer == 1 && posX<=1) {
            if (Board.cardMatriz[2, posY] != null) {
                ...
                target.transform.FindChild("HP").GetComponent<Text>().text = target.GetComponent<CardAttack>().HP.ToString();
                target.transform.FindChild("HP").GetComponent<Text>().color = Color.red;
            } else {
